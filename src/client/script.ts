declare var _: any;  // Declare Lodash as a global variable of type 'any'

// Define the shape of a comment object using TypeScript interfaces
interface Comment {
    id: number;
    postId: number;
    name: string;
...
const resultsContainer = document.getElementById('results-wrapper') as HTMLElement | null;

let cachedComments: Comment[] = []; // Cache to store fetched comments

let debounceTimer: NodeJS.Timeout;

// Function to fetch comments from the API and cache them
async function fetchComments(): Promise<Comment[]> {
    if (cachedComments.length > 0) return cachedComments; // Use cached data if available
